...
{
	// We have precomputed that, in the grid below, the greatest product
	// of 4 consecutive numbers is 70600674. Verify that we get the right
	// answer in order to test ENumberGrid.

	constexpr std::size_t TEST_GRID_SIZE = 20;
	constexpr int values[TEST_GRID_SIZE][TEST_GRID_SIZE] = {
	...
	constexpr int EXPECTED_RESULT = 70600674;

	ENumberGrid g(20);
	for(std::size_t i = 0; i < TEST_GRID_SIZE; ++i)
	{
		for(std::size_t j = 0; j < TEST_GRID_SIZE; ++j)
		{
			g.set(static_cast<int>(i), static_cast<int>(j),
			      values[i][j]);
		}
	}

	CHECK(g.greatestConsecutiveProduct(4) == EXPECTED_RESULT);
}
