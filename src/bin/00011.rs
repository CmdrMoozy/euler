...
fn get_back_diagonal_line_product(grid: &[&[u64]], origin: (usize, usize), length: usize) -> u64 {
    let mut product: u64 = grid[origin.1][origin.0];
    for off in 1..length {
        product *= grid[origin.1 + off][origin.0 + off];
    }
    product
}
...
fn get_horizontal_line_product(grid: &[&[u64]], origin: (usize, usize), length: usize) -> u64 {
    let mut product: u64 = grid[origin.1][origin.0];
    for off in 1..length {
        product *= grid[origin.1][origin.0 + off];
    }
    product
}

fn get_vertical_line_product(grid: &[&[u64]], origin: (usize, usize), length: usize) -> u64 {
    let mut product: u64 = grid[origin.1][origin.0];
    for off in 1..length {
        product *= grid[origin.1 + off][origin.0];
    }
    product
}
...
fn main() {
    main_impl(|| -> Result<ProblemAnswer<u64>> {
        ...
        // The above loop skips the final (LINE_LENGTH - 1) horizontal and vertical
        // rows.
