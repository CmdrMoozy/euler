...
constexpr std::size_t GRID_SIZE = 20;
constexpr int GRID_VALUES[GRID_SIZE][GRID_SIZE] = {
...
{
	ENumberGrid grid(GRID_SIZE);
	for(std::size_t i = 0; i < GRID_SIZE; ++i)
	{
		for(std::size_t j = 0; j < GRID_SIZE; ++j)
		{
			grid.set(static_cast<int>(i), static_cast<int>(j),
			         GRID_VALUES[i][j]);
		}
	}

	return {grid.greatestConsecutiveProduct(4), EXPECTED_RESULT};
}
}
