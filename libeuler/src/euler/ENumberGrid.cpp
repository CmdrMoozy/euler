...
void ENumberGrid::doTestSuite()
{
	...
	try
	{
		...
		/*
		 * Since we only have one function, just test it by solving a problem. It is known that
		 * the greatest product of 4 consecutive numbers in the given grid is 70600674 - make sure
	...
	throw(EOutOfBoundsException &)
{
	...
	if(n > getSize())
		throw EOutOfBoundsException("The consecutive length provided is out-of-range.");
	...
	try
	{
		...
		// Our loop skips the final (n-1) horizontal and vertical rows. Test them.
