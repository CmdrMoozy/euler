 ...
 *
 * In general, we store an angle measurement in degrees, and the "direction" it goes in is stored
 * (this is more or less the same as storing whether we are a positive/negative latitude/longitude
 * value). We provide some conversion functions, but serious math should probably be done via
 * QGeoCoord.
 *
 * Note that these values are created, by default, with QLatLonValue::DEFAULT_PRECISION bits of
 * precision. If you need to change this, you should do so with setPrecision() before using the
 * object for anything else.
 *
 * The default, 128 bits, should be sufficient for most applications, however.
 *
 * It is also worth noting that, instead of having a test suite function ourselves, we depend on
 * EGeoCoord to test our objects. Since it does math with us, all of our functions are utilized
...
class ELatLonValue
{
	public:
		/*!
		 * \brief This constant denotes the default number of bits of precision our class uses.
		 */
		static const mpfr_prec_t DEFAULT_PRECISION = 128;

		/*!
		 * \brief This enumeration indicates a type of angle measurement - either degrees or radians.
		 */
		enum ValueType
		{
			Degrees,
		...
		/*!
		 * \brief This enumeration denotes an angle direction - NSEW, or "None" for zero longitude/latitude.
		 */
		enum ValueDirection
		{
			North,
