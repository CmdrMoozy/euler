 ...
 *
 * It must implement an () operator which returns the resulting hash as a
 *uint32_t, and accepts a
 * key value, and a uint32_t value r representing the number of bits desired in
 *the resulting hash.
 */
struct Hash
{
	...
	{
		size_t i;
		uint32_t v = 0xb11924e1; // Start with some large, random prime
		                         // number.

		/*
		 * We expand each individual character, so even a very short
		 * (e.g, 1-character-long) string will
		...
		for(i = 0; i < k.size(); ++i)
		{
			if(k[i] & 1)
			{
				...
				v += static_cast<uint32_t>(
				             Hash::rotateLeft(k[i], 3))
				     << 16;
				v += static_cast<uint32_t>(
				             Hash::rotateRight(k[i], 5))
				     << 24;
			}
			else
			{
				...
				v += static_cast<uint32_t>(k[i]) << 8;
				v += static_cast<uint32_t>(
				             Hash::rotateLeft(k[i], 4))
				     << 16;
				v += static_cast<uint32_t>(
				             Hash::rotateRight(k[i], 6))
				     << 24;
			}
		}
	...
	/*!
	 * This function uses a multiplicative method of hashing (see
	 *"Introduction to Algorithms," pp. 231),
	 * returning (A*k % 2^w) >> (w-r), where A is a constant, k is the key
	 *value we are trying to hash,
	 * w is the word size of our hardware (this code enforces a word-size of
	 *32-bits), and our table contains
	 * 2^r slots.
	 *
	 * Assuming our table does indeed contain 2^r slots, and r is passed
	 *correctly, this function will never
	...
	uint32_t operator()(uint32_t k, uint32_t r)
	{
		...
		return (((Hash::A * k) % (~static_cast<uint32_t>(0))) >>
		        (32 - r));
	}

private:
	/*
	 * For this hashing method, A should be an ODD constant that is
	 * reasonably far away from
	 * both 2^31 and 2^32.
	 */
	static const uint32_t A = 0xbf8d031b;

	/*!
	 * This function performs a circular left shift of the given char value
	 *by n places.
	 *
	...
	/*!
	 * This function performs a circular right shift of the given char value
	 *by n places.
	 *
	 * \param c The char value we will be modifying.
	 * \param n The number of places to shift the value.
 ...
 *
 * All operations should, on average, operate in constant (i.e., O(1)) time,
 *assuming the load factor
 * of the hash table (that is, the ratio of keys being stored to the size of the
 *table) is < 1.0, assuming
 * that our hash function distributes everything evenly (this should be
 *relatively true in most common-use
 * cases).
 */
template <typename Key, typename Value, class Hasher = Hash> class EHashMap
{
public:
#ifdef LIBEULER_DEBUG
	/*!
	...
	static void doTestSuite()
	{
		...
		try
		{
			...
			// Clear our map and add a whole bunch of items to it,
			// to test our rehash() function.

			for(i = 0; i < 50000; ++i)
				m.put(i, i);

			EASSERT(m.getKeys() == 50000)
			EASSERT(m.getDesiredLoadFactor() >
			        m.getCurrentLoadFactor())

			for(i = 0; i < 50000; ++i)
				EASSERT(m.containsKey(i) && (m.get(i) == i))

			// Test our copy constructor and copy assignment
			// operator.
	...
	/*!
	 * This is our default constructor, which initializes a new SHashMap
	 *object with the given
	 * initial bit-length (the initial capacity will be 2^r) and the given
	 *desired load factor.
	 * Our hash map resizes itself automatically in order to maintain the
	 *given desired load factor,
	 * but this operation is quite expensive, so you should try to choose a
	 *reasonable initial
	 * capacity and/or load factor for the specific case.
	 *
	 * The load factor is defined as the ratio of stored keys to the total
	 *capacity.
	 *
	...
	EHashMap(uint32_t r = 10, float l = 0.75) : bits(r), load(l), keys(0)
	{
		int i;
		capacity =
		        static_cast<int>(euler::math::ipow(2, getHashBits()));
	...
	/*!
	 * This is our copy constructor, which will initialize our object to be
	...
	/*!
	 * This is our default destructor, which frees all of our contents so
	 * our object can be cleaned
	 * up and destroyed.
	 */
	virtual ~EHashMap()
	{
	...
	/*!
	 * This is our assignment operator, which sets our hash map's value
	 *equal to that of the given
	 * other hash map.
	 *
	 * \param o The other hash map to make ourself equal to.
	...
	/*!
	 * This function returns our current desired load factor. The load
	 *factor is defined
	 * as a ratio of the number of keys stored to the total capacity of the
	 *hash map.
	 *
	...
	/*!
	 * This function sets our desired load factor. The load factor is
	 *defined as a ratio
	 * of the number of keys stored to the total capacity of the hash map.
	 *This function
	 * can result in a rehash operation (which is quite expensive), if the
	 *given load factor
	...
	/*!
	 * This function returns our calculated current load factor. The load
	 *factor is defined
	 * as a ratio of the number of keys stored to the total current capacity
	 *of our hash map.
	 *
	...
	{
		return (static_cast<float>(keys) /
		        static_cast<float>(getCapacity()));
	}
	...
	/*!
	 * This function returns the capacity of our hash map (i.e., the number
	 *of distinct keys
	 * that could be stored without creating lists). The square root of this
	 *value is the bit
	 * length of our hash.
	 *
	...
	/*!
	 * This function returns the number of distinct keys our hash map is
	 *currently storing.
	 *
	...
	/*!
	 * This function clears our list of all keys (but doesn't modify our
	 * hash bit length, our
	...
	/*!
	 * This is a convenience function, which tests whether or not our map is
	 *empty.
	 *
	...
	/*!
	 * This function returns the value that is mapped to by the given key.
	 *This function should operate
	 * in constant (O(1)) time, assuming a good hash distribution is
	 *achieved.
	 *
	 * \param k The key to search for.
	 * \exception EOutOfBoundsException This exception is thrown if our map
	 *doesn't contain the given key.
	 * \return The value that corresponds to the given key.
	 */
	virtual Value &get(const Key &k) const throw(EOutOfBoundsException &)
	{
		...
		if(n != NULL)
			...
			throw EOutOfBoundsException(
			        "The QHashMap doesn't contain the given key.");
	}

	/*!
	 * This function inserts the given value for the given key. If a value
	 *for that key is already present,
	 * then we just replace it with the given new value.
	 *
	 * \param k The key for which to insert our value.
	 * \param v The value to insert.
	 * \return True if a new key was inserted, or false if the value was
	 *merely updated.
	 */
	...
	/*!
	 * This function erases the given key from our hash map, if it is
	 *present.
	 *
	 * \param k The key to search for.
	 * \return True if the key was removed, or false if it was never present
	 *to begin with.
	 */
	...
	/*!
	 * This is a convenience function which trades our values with that of
	 *the given
	 * other object. Since this function just trades pointers, it operates
	 *in O(1) time.
	 *
...
private:
	 ...
	 *
	 * Pointers to this class are what our BucketList class stores. This
	 *class contains the key and value
	 * pair that was inserted into our hash table, and also a pointer to the
	 *next Node in the BucketList.
	 */
	class Node
	{
	public:
#ifdef LIBEULER_DEBUG
		/*!
		 * This function implements our test suite for this class. It
		 * uses non-abort()'ing
		 * assertions, and merely prints the result to stdout.
		 */
		static void doTestSuite()
		...
		/*!
		 * This is our default constructor, which initializes a new Node
		 *object with (optionally) the
		...
		/*!
		 * This is our copy constructor, which initializes our object to
		 *be equal
		...
		/*!
		 * This is our default destructor. To make it easier to free up
		 * our hash table, this destructor
		 * will delete our next node as well if it is non-NULL, which
		 * will in turn delete its next node,
		 * and so on. What this means is, if you do not want any other
		 * nodes to be deleted, you need to
		...
		/*!
		 * This function returns a pointer to the node after this one in
		 *a BucketList, or NULL if no such
		 * node exists.
		 *
	 ...
	 *
	 * In order to handle hash collisions, our hash table utilizes a scheme
	 *called "separate chaining"
	 * (http://en.wikipedia.org/wiki/Hash_table#Separate_chaining), where
	 *each slot in our table is a
	 * singly-linked list, and any collisions are simply appended to that
	 *list.
	 */
	class BucketList
	{
	public:
		...
		static void doTestSuite()
		{
			...
			try
			{
				...
				// Make sure insert overrides existing keys
				// correctly.
		 ...
		 * \param o The other object to make ourself equal to.
		 */
		BucketList(const BucketList &o) : head(NULL)
		{
			(*this) = o;
		}

		/*!
		 * This is our default destructor, which deletes every element
		 * of our list recursively,
		 * so out object can be cleaned up & destroyed.
		 */
		virtual ~BucketList()
		{
			// We delete our head node, which should in turn
		...
		/*!
		 * This is our assignment operator, which will set our value
		 *equal to that
		 * of the given other object, deleting any existing data we are
		 *storing in
		 * the process.
		 *
		 * \param o The other object to make ourself equal to.
		...
		/*!
		 * This function inserts the given key/value pair into our list.
		 *If the list already contains
		 * the given key, then its corresponding value is simply
		 *updated, or otherwise, we add a brand
		 * new node.
		 *
		 * \param k The key for this node.
		 * \param v The value for this node.
		 * \return True if a new node was created, or false if a value
		 *was simply updated.
		 */
		...
		/*!
		 * This function erases the node with the given key value from
		 *our list, and returns whether
		...
		/*!
		 * This function does a linear search through our list, and
		 *returns a pointer to the node with the
		 * given key value, or NULL if one could not be found.
		 *
		 * \param k The key value we are searching for.
		 * \return A pointer to the node with the given key, or NULL if
		 *no such node exists.
		 */
		...
		/*!
		 * This function returns a pointer to our head element (for
		 *instance, so the
		...
		/*!
		 * This function returns the number of nodes this BucketList
		 *contains.
		 *
	...
	/*!
	 * This is a convenience function, which calculates the minimum number
	 *of hash-bits required
	 * to store the given number of keys while maintaining the given desired
	 *load factor.
	 *
	...
	/*!
	 * This internal function rehashes all of our keys, generally to
	 * maintain our desired
	 * load factor. If we are already within that number, then no action is
	 * taken.
	 */
	void rehash()
	{
		...
		EHashMap<Key, Value, Hasher> newTable(
		        getMinimumBits(getHashBits(), getKeys(),
		                       getDesiredLoadFactor()),
		        getDesiredLoadFactor());

		// Insert all of our existing key/value pairs into the new
		// table.
		for(i = 0; i < getCapacity(); ++i)
		{
			if(array[i] == NULL)
