 ...
 *
 * It is a very simple implementation of a two-dimensional array that does bounds
...
class EGrid
{
	public:
		 ...
		 * (optionally) with the given size. Note that negative sizes are treated as
		 * zero.
		 *
		...
		/*!
		 * This function just returns our grid's current size. Note that this is the size
		 * in both dimensions, i.e., our grid is n by n, where n is the size returned by
		 * this function.
		 *
		...
		/*!
		 * This function sets the current size of our grid. Note that previous contents are
		...
		/*!
		 * This function clears all of the contents of our grid. It is equivalent in functionality
		 * to setSize(0).
		 */
		void clear()
		{
		...
		/*!
		 * This function returns a copy of the value stored at the given position in our grid.
		 *
		 * \param i The row at which the given value is found.
		 * \param j The column at which the given value is found.
		 * \exception QOutOfBoundsException This exception is thrown if the given row and/or column is out-of-bounds.
	...
	private:
		...
		/*!
		 * This is a utility function that we use for bounds-checking, so we don't have to duplicate
