...
class EArrayUtilities
{
	public:
		...
		static void doTestSuite()
		{
			...
			try
			{
				...
				int psc = static_cast<int>( pow( 2.0, static_cast<double>(os.size()) ) );
				std::set< std::set<int> > ps = EArrayUtilities::getPowerSet<int>(os);
				...
				for(i = 0; i < 9; ++i)
				{
					...
					bI = EArrayUtilities::binarySearch<int>(b, 0, 9, b[i]);
					
					EASSERT(aI == i)
					EASSERT(bI == i)
					
					aI = EArrayUtilities::search<int>(a, 10, i);
					bI = EArrayUtilities::binarySearch<int>(b, 0, 9, i);
		...
		template<typename T>
		static std::multiset< std::multiset<T> > getPowerSet(const std::multiset<T> &s)
		{
			...
			struct operators
			{
				static T dereference(typename std::multiset<T>::const_iterator it) { return (*it); }
			};
			
			do
			{
				tmp.clear();
				std::transform(elements.begin(), elements.end(), std::inserter(tmp, tmp.end()),
		...
		/*!
		 * This function calculates and returns the power set of the given set s. That is, the set of all
		 * subsets of s -- which will be 2^|s| in size.
		 *
		 * This implementation is courtesy of a pretty straight-forward implementation provided on RosettaCode:
		...
		static std::set< std::set<T> > getPowerSet(const std::set<T> &s)
		{
			...
			struct operators
			{
				static T dereference(typename std::set<T>::const_iterator it) { return (*it); }
			};
		...
		/*!
		 * This function just copies the given array, returning a pointer to a brand-new identical array.
		 *
		...
		/*!
		 * This function tests whether or not the two arrays given are exactly equal, including ordering.
		 * Note that the length of both arrays is assumed to be the same, since arrays of different sizes
		 ...
		 */
		template<typename T>
		static bool equal(const T *a, const T *b, int l)
		{
			int i;
			...
			// Just check each element in order until we find one that isn't equal.
			for(i = 0; i < l; ++i)
				if(a[i] != b[i])
					return false;
		...
		/*!
		 * This function tests whether or not the two arrays are equal, ignoring sorting. Note that this function
		 * is somewhat slow, since it achieves this by simply sorting the arrays and then namking the comparison.
		 * Note that the length of both arrays is assumed to be the same, since arrays of different sizes
		 * are obviously not equal.
		 *
		 * \param a The first array.
		...
		/*!
		 * This function resizes the given array to a given new size, optionally preserving existing data.
		 *
		...
		/*!
		 * This function sorts the given array of given length in ascending order.
		 * 
		 * \param a The array to sort.
		 * \param l The length of the array to sort.
		 */
		template<typename T>
		static void sortAscending(T *a, int l)
		{
			quicksortAsc<T>(a, 0, l-1);
		}
		
		/*!
		 * This function sorts the given array of given length in descending order.
		 * 
		 * \param a The array to sort.
		 * \param l The length of the array to sort.
		 */
		template<typename T>
		static void sortDescending(T *a, int l)
		{
			quicksortDesc<T>(a, 0, l-1);
		}
		
		/*!
		 * This function reverses the given array from the index l to the index r (inclusive).
		 *
		 ...
		 */
		template<typename T>
		static void reverse(T *a, int l, int r)
		{
			T hold;
			while(l < r)
			{
		...
		/*!
		 * This function just does a linear search for the given needle, returning the first
		 * occurrence of it.
		 *
		 * \param a The array to search.
		 * \param l The length of the array.
		 * \param n The needle we are searching for.
		 * \return The index of the first instance of the needle, or -1 if it wasn't found.
		 */
		template<typename T>
		static int search(const T *a, int l, const T &n)
		{
			int i;
		...
		/*!
		 * This function does a binary search for a given needle in the given array. It operates
		 * much faster than search(), but it requires that the array is sorted in ascending order
		 * first. If the array is not sorted, then the behavior of this function is undefined.
		 *
		 * \param a The array to search.
		 * \param l The left index.
		 * \param r The right index.
		...
		/*!
		 * This function tests whether or not the given array contains duplicate elements or not.
		 *
		 ...
		 */
		template<typename T>
		static bool isUnique(const T *a, int l)
		{
			int i;
			T *s;
		 ...
		 */
		template<typename T>
		static int makeUnique(T *&a, int l)
		{
			int i, j, n;
			T *tmp;
		 ...
		 */
		template<typename T>
		static bool permutate(T *a, int s)
		{
			T hold;
			int k, l, i;
		 ...
		 */
		template<typename T>
		static bool reversePermutate(T *a, int s)
		{
			T hold;
			int k, l, i;
	...
	private:
		 ...
		 */
		template<typename T>
		static void quicksortAsc(T *a, int l, int r)
		{
			T pivot, hold;
			int i, j;
		 ...
		 */
		template<typename T>
		static void quicksortDesc(T *a, int l, int r)
		{
			T pivot, hold;
			int i, j;
