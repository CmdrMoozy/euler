 ...
 *search bounds. The lower
 * bound is obvious; the problem says n! = n (where n is a 1-digit number)
 *doesn't count, since that
 ...
 *
 * 9! * d is the largest number we could have, since, for instance, 999 would be
 *9! + 9! + 9! (or 9! * 3).
 *
 ...
 *possible factorion is a 7-digit
 * number. However, 9,999,999 is larger than the upper bound at d=7, so our real
 *upper bound is 9! * 7,
 * or 2,540,160.
 */
...
int main(void)
{
	...
	uint32_t factorials[10] = {1,   1,   2,    6,     24,
	                           120, 720, 5040, 40320, 362880};

	/*
	 * Note that we are not including "isFactorion()" or similar in libeuler
	 * because a) using a lookup
	 * table greatly improves our speed, and doing so isn't very clean in a
	 * static function, and b) it
	...
	for(result = 0, i = LOWER_BOUND; i <= UPPER_BOUND; i++)
	{
		...
		// If the sum is equal to the original, this is a factorion; add
		// it to the result.
		if(sum == i)
			result += sum;
	}
