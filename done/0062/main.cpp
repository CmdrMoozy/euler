...
int main(void)
{
	...
	// We can start with 8-digit cubes, since we're given that 41063625 is
	// the smallest 3-permutation.
	int digits = 8;

	while(true)
	{
		// Build the range of numbers we'll be testing for this digit
		// count.
		...
		/*
		 * For each base in this range, compute its cube root. For each
		 *other value in this
		 * range, if it's a permutation of the current value, we count
		 *it.
		 *
		 * If this value has exactly CUBIC_PERMS_TARGET permutations
		 *which are cubes in the same
		 * range, we're done. Otherwise, we can remove ALL permutations
		 *of this value from our
		 * list, since they have already been checked by proxy.
		 */

		for(std::vector<uint64_t>::iterator it = bases.begin();
		    it != bases.end(); ++it)
		{
			...
			// For each other value, test if it's a permutation of
			// this value.

			std::vector<std::vector<uint64_t>::iterator> perms;

			for(std::vector<uint64_t>::iterator oit = it + 1;
			    oit != bases.end(); ++oit)
			{
				uint64_t ov = (*oit) * (*oit) * (*oit);

				// If this other value is a permutation, add it
			...
			// If we found enough other permutations, we're done.
			// Otherwise, skip those others.

			if(perms.size() == (CUBIC_PERMS_TARGET - 1))
			{
				// Set our result, and stop looping - we're
				// done!
			...
			{
				// This value isn't the answer - clear it and
				// all of its permutations.

				(*it) = 0;

				for(std::vector<std::vector<
				            uint64_t>::iterator>::iterator cit =
				            perms.begin();
				    cit != perms.end(); ++cit)
				{
					(**cit) = 0;
