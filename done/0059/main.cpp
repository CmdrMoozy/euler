 ...
 *standard is ASCII
 * (American Standard Code for Information Exchange). For example, uppercase A =
 *65, asterisk
 * (*) = 42, and lowercase k = 107.
 *
 * A modern encryption method is to take a text file, convert the bytes to
 *ASCII, then XOR each
 * byte with a given value, taken from a secret key. The advantage with the XOR
 *function is that
 ...
 *message, and the key
 * is made up of random bytes. The user would keep the encrypted message and the
 *encryption key
 * in different locations, and without both "halves", it is impossible to
 *decrypt the message.
 *
		...
		for(key[1] = 'a'; key[1] <= 'z'; key[1]++)
		{
			for(key[2] = 'a'; key[2] <= 'z'; key[2]++)
			{
				...
				for(i = 0; i < ovalues.size(); i++)
				{
					...
					/*
					 * We are given that the resulting
					 * message contains only
					 * ASCII-characters, so for each
					 * character we make sure it is in range
					 * [32], [65, 90], [97, 122]. If we get
					 * a resultant
					 * value that is not, then we just
					 * continue to the next key.
					 */

					if(!isalnum(c) && !isPunctuation(c) &&
					   !isspace(c))
					{
						fail = true;
						break;
				...
				// If we decrypted the whole message
				// successfully, we need to add up the values of
