...
class DigitalFraction
{
	public:
		/*
		 * This is our default constructor, which initializes a new, clear()'ed DigitalFraction.
		 */
		DigitalFraction()
		{
			clear();
		...
		/*
		 * This constructor initializes a new DigitalFraction with the given numerator/denominator.
		 */
		DigitalFraction(uint32_t n, uint32_t d)
			: numerator(n), denominator(d)
		{
		}
		
		/*
		 * This is our copy constructor, which initializes our object to be equal to the given other
		 * object.
		 */
		...
		/*
		 * This is our copy assignment operator; it makes our object equal to the given other object,
		...
		/*
		 * This function tests direct equivalence between us and the given other DigitalFraction. This
		 * means that we directly compare numerator and denominator; cancelling / reducing is not taken
		 * into account.
		 */
		...
		/*
		 * A digital fraction is considered "trivial" if both the numerator and denominator are multiples
		 * of 10.
		 */
		bool isTrivial() const
		{
			return ( ((numerator.toInteger() % 10) == 0) && ((denominator.toInteger() % 10) == 0) );
		}
		
		/*
		 * This function does a digit-wise cancel of our fraction. For instance: 49/98 will become
		 * 4/8. Note that this is, by design, incorrect math, which is what this problem requires to
		 * solve.
		 */
		void cancelAll()
		{
			std::string n = numerator.toString(), d = denominator.toString();
			int i, j;
			bool reduced;
		...
		/*
		 * This function returns the decimal value of our fraction -- the numerator divided by the denominator.
		 */
		double toDecimal() const
		{
			return (static_cast<double>(numerator.toInteger()) / static_cast<double>(denominator.toInteger()));
		}
...
int main(void)
{
	...
	for(n = 0; n < fractions.size(); n++)
	{
		...
		f.setDenominator(f.getDenominator() * fractions[n].getDenominator());
	}
	
	f.reduce();
	
	result = f.getDenominator();
	std::cout << "The denominator of the resulting fraction product is: " << result << "\n";
