...
{
	int i, j, greatest;
	int gridValues[20][20] = {{8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7,
	                           78, 52, 12, 50, 77, 91, 8},
	...
	for(i = 0; i < 20; i++)
		for(j = 0; j < 20; j++)
			grid.set(i, j, gridValues[i][j]);

	greatest = grid.greatestConsecutiveProduct(4);
	std::cout << "The greatest product of four consecutive digits (in any "
	             "direction) "
	          << "is: " << greatest << "\n";

	assert(greatest == 70600674);
	return 0;
}
