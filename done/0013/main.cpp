...
int main(void)
{
	...
	while(in.good())
	{
		...
		// Make sure the input numbers we are reading are 50-digits
		// long.
		if(buf.length() != 50)
		{
	...
	// Loop through all of the digits we read in, adding them in our add
	// buffer.
	...
	// Continue carrying, but this time we drop off the right-most place as
	// needed to keep our buffer the right size.
	while(addBuf[0] >= 10)
	{
		// Shift our buffer right 1 place, discarding the right-most
		// digit.
		for(i = 49; i > 0; i--)
			addBuf[i] = addBuf[i - 1];
	...
	std::cout << "The first ten digits of our resulting number after "
	             "adding one hundred 50-digit numbers are: ";
