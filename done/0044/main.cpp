 ...
 *(x` + y`) and (x` - y`) equal
 * to the factors a and b (where a > b; if b > a, the factors are swapped), and
 ...
 *the system, instead just
 * gives the values of x and y directly. This is because the coefficients in the
 *systems are always the
 * same; it is only what the system is equal to that changes, so we can get a
 ...
 *     x` - y` = b
 *
 * By solving this system of equations, we can see that x` = (a+b)/2 and y` =
 *(a-b)/2. We then set the
 * values x` and y` represent equal to these values, and solve for the
 *respective variable to arrive at
 * the final solution.
 *
 ...
 *that 3*n + 1 > P(d),
 * which (for this particular problem) equates to around four billion loop
 *iterations.
 *
 * This approach is to iterate over each pentagonal number, as before, but then
 *to express that pentagonal
 * number as the difference of pentagonal numbers, and to solve the resulting
 ...
 *
 * Next, we complete the square on the first two terms by adding and subtracting
 *1:
 *
 ...
 *we solve where (x` + y`) is
 * equal to the larger factor, and (x` - y`) is equal to the smaller factor. If
 *any of these solutions are
 * such that both x and y are positive integers, then we have found the indices
 *of the two pentagonal numbers
 ...
 */
	...
	std::cout << "The smallest pentagonal difference is: "
	          << EMath::getPentagonalNumberN(context.result) << "\n";
					...
					if(QMath::isPentagonal(
					           ...
					           QMath::getPentagonalNumberN(
					                   static_cast<
					                           ...
					                           y))))
					{
						// Print out the answer - we're
						// done!

						std::cout << "The smallest "
						             "pentagonal "
						             "difference is: "
						          << pD << "\n";
						assert(d == 1912);
