...
int main(void)
{
	...
	{ // For each odd composite n...
		...
		while(n > sieve.getLimit())
			sieve.setLimit(sieve.getLimit() *
			               SIEVE_LIMIT_MULTIPLIER);
		...
		isResult = true;
		for(sit = sieve.begin(); (sit != sieve.end()) && ((*sit) < n);
		    ++sit)
		{ // For each prime less than n...
			...
			// The difference must be twice a square, so if it is
			// not even then continue.
			if((d & 1))
				continue;
			...
			// Test if this particular prime can be written as twice
			// a square.
			d /= 2;
