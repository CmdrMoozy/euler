...
constexpr uint64_t EXPECTED_RESULT = 100;
...
public:
	 ...
	 * equal to the given other object.
	 */
	 ...
	 * the given other object, and then returns a reference to this so the
	 ...
	 * DigitalFraction. This means that we directly compare numerator and
	 * denominator; cancelling / reducing is not taken into account.
	 */
	 ...
	 * denominator are multiples of 10.
	 */
	 ...
	 * 49/98 will become 4/8. Note that this is, by design, incorrect math,
	 * which is what this problem requires to solve.
	 */
	void cancelAll()
	{
...
euler::util::process::ProblemResult<uint64_t> problem()
{
	...
	for(d = 10; d <= 99; d++)
	{
		for(n = 10; n < d; n++)
		{
			...
			if(!(a == b) && (a.getDenominator() != 0) &&
			   (b.getDenominator() != 0) &&
			   (std::abs(a.toDecimal() - b.toDecimal()) <
	...
	f.reduce();
	return {f.getDenominator(), EXPECTED_RESULT};
}
}
