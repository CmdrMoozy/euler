 ...
 * search bounds. The lower bound is obvious; the problem says n! = n (where
 * n is a 1-digit number) doesn't count, since that is not a sum, so we can
 * start at 10.
 *
...
constexpr uint32_t UPPER_BOUND = 2540160;

constexpr uint32_t EXPECTED_RESULT = 40730;
